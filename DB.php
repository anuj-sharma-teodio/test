...
class DB {
     ...
     * @param array select, where, order_by, limit and return_type conditions
     */
    public function aunthenticateUser($table,$user,$conditions = array()){
        $sql = 'SELECT ';
        $sql .= array_key_exists("select",$conditions)?$conditions['select']:'*';
        $sql .= ' FROM '.$table.' where email="'.$user['email'].'" and password="'.md5($user['password']).'"';
